...
@pytest.mark.asyncio
async def test_create_ar_ledger_and_list(async_client: AsyncClient, setup_tenant, auth_headers):
    payload = {
        "tenant_id": str(setup_tenant.id),
        "customer_id": str(setup_tenant.id),
...
@pytest.mark.asyncio
async def test_ar_collections_and_receipts(async_client: AsyncClient, setup_tenant, auth_headers):
    coll_payload = {
        "tenant_id": str(setup_tenant.id),
        "customer_id": str(setup_tenant.id),
...
@pytest.mark.asyncio
async def test_bank_and_expense(async_client: AsyncClient, setup_tenant, auth_headers):
    br_payload = {
        "tenant_id": str(setup_tenant.id),
        "bank_account_id": str(setup_tenant.id),
