...
async def test_batch_and_serial_tracking(async_client: AsyncClient, auth_headers: dict):
    ...
    item_res = await async_client.post("/api/v1/universal-inventory/items", json={"item_code": f"ITM-{uuid.uuid4()}", "name": "Item", "category_id": cat_res.json()["id"]}, headers=auth_headers)
