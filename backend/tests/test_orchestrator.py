    ...
    async with db_session() as db:
        ...
        if permissions:
            ...
            for module, feature, action in permissions:
                ...
                # create rather than blind-insert, since a full-suite run
                # has many tests requesting the same tuple and this file's
                # own cleanup never deletes catalog rows (correctly --
                # other tests running concurrently/afterward may still
                # need them). .limit(1) ahead of .scalar_one_or_none():
                # belt-and-suspenders against duplicates ever being
                # reintroduced despite the constraint (same pattern
                # app/middleware/rbac.py's check_permission already uses).
