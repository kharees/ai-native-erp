...
class MigrationExecutionManager:
    ...
    @staticmethod
    async def start_execution(db: AsyncSession, tenant_id: uuid.UUID, session_id: uuid.UUID):
    ...
    @staticmethod
    async def pause_execution(db: AsyncSession, tenant_id: uuid.UUID, session_id: uuid.UUID):
        stmt = select(MigrationSession).where(
            MigrationSession.id == session_id, MigrationSession.tenant_id == tenant_id
        )
        session = (await db.execute(stmt)).scalar_one_or_none()
    ...
    @staticmethod
    async def resume_execution(db: AsyncSession, tenant_id: uuid.UUID, session_id: uuid.UUID):
        stmt = select(MigrationSession).where(
            MigrationSession.id == session_id, MigrationSession.tenant_id == tenant_id
        )
        session = (await db.execute(stmt)).scalar_one_or_none()
    ...
    @staticmethod
    async def cancel_execution(db: AsyncSession, tenant_id: uuid.UUID, session_id: uuid.UUID):
        stmt = select(MigrationSession).where(
            MigrationSession.id == session_id, MigrationSession.tenant_id == tenant_id
        )
        session = (await db.execute(stmt)).scalar_one_or_none()
...
class MigrationRollbackEngine:
    ...
    async def rollback_session(
        ...
        partial: bool = False, record_ids: list = None
    ):
        """
        ...
        # session_id alone would be enough to reach the right rows (records
        # are children of an already tenant-verified session), but the
        # explicit tenant_id filter here matches the pattern used everywhere
        # else in this file — one less thing to get wrong if this query is
        # ever copied elsewhere without the session ownership check above it.
        records_stmt = select(MigrationDataRecord).where(
            MigrationDataRecord.session_id == session_id, MigrationDataRecord.is_imported == True
        )
...
class MigrationReconciliationEngine:
    ...
    @staticmethod
    async def generate_report(db: AsyncSession, tenant_id: uuid.UUID, session_id: uuid.UUID):
        """
